...
class RepoStore {
  ...
  public async add(repo: Repository): Promise<string> {
    try {
      const existingRepo = _.find(
        this.repos,
        (r) => r.fullpath === repo.fullpath
      );
      if (existingRepo) {
      ...
      throw new Error(
        `Failed to stats repository (${repo.fullpath}) error: ` + e.stack
      );
    }
    if (!repo.id) {
      repo.id = await getRepoId(
        repo,
        this.getRepositories().map((r) => r.id)
      );
    }
  ...
  public remove(id: string): boolean {
    ...
    removed.forEach((r) => {
      ...
      ipcRenderer.send("track", "repo-remove", {
        repoName: r.repoName,
        repoId: r.id,
      });
    });
