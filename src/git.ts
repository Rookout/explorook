...
export async function getRepoId(
  ...
  try {
    ...
    const remote = await igit.config({
      fs,
      dir: gitRoot,
...
export const getLastCommitDescription = async (
  repo: Repository
): Promise<igit.CommitDescription> => {
  try {
    let gitRoot = null;
    try {
...
export const getRemoteOriginForRepo = async (
  repo: Repository
): Promise<igit.RemoteDescription> => {
  try {
    let gitRoot = null;
    try {
      gitRoot = await igit.findRoot({ fs, filepath: repo.fullpath });
...
export async function getCommitIfRightOrigin(
  repo: Repository,
  remoteOrigin: string
): Promise<string> {
  ...
  return parsedLocalRemoteOrigin.name === argsParsedRemoteOrigin.name &&
    parsedLocalRemoteOrigin.owner === argsParsedRemoteOrigin.owner
    ? (await getLastCommitDescription(repo))?.oid
    : null;
}
...
export async function cloneRemoteOriginWithCommit(
  repoUrl: string,
  commit: string,
  isDuplicate: boolean
) {
  ...
  const gitRoot = isDuplicate
    ? path.join(GIT_ROOT, `${TMP_DIR_PREFIX}${uuidv4()}`)
    : GIT_ROOT;
  ...
  const fullCommand = doesRepoExist
    ? checkoutCommand
    : `${cloneCommand} && ${checkoutCommand}`;
