...
ipcRenderer.once(
  "exception-manager-enabled-changed",
  (e: IpcRendererEvent, enabled: boolean) => {
    if (enabled) {
...
ipcRenderer.on(
  "edit-repo",
  (e: IpcRendererEvent, args: { id: string; repoName: string }) => {
...
ipcRenderer.on(
  ...
  (e: IpcRendererEvent, connectionString: string) => {
    let isSuccess = false;
    try {
      isSuccess = !!changePerforceManagerSingleton(connectionString);
    } catch (err) {
      console.error(
        `Failed to init perforce manager with port :${connectionString}`,
        err
      );
    }
    ...
    ipcRenderer.sendTo(
      mainWindowId,
      "test-perforce-connection-result",
      isSuccess
    );
  }
);

ipcRenderer.on("repos-request", (e: IpcRendererEvent) =>
  ipcRenderer.sendTo(mainWindowId, "refresh-repos", getRepos())
);
