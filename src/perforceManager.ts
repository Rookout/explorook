...
const ROOT =
  process.platform === "win32"
    ? path.join(process.env.APPDATA, "\\Rookout\\Perforce_Root")
    : path.join(
        process.env.HOME,
        "Library/Application Support/Rookout/Perforce_Root"
      );
...
class PerforceManager {
  ...
  constructor(perforceConnectionString: string) {
    ...
    if (
      !_.find(
        allWorkspaces,
        (workspace) => workspace.Client === currentRookoutClientName
      )
    ) {
  ...
  public async changeViews(views: string[]): Promise<IPerforceRepo[]> {
    ...
    for (let i = 0; i < targetViews.length; i++) {
      ...
      client[
        "View" + i
      ] = `//${targetViews[i].name}/... //${client.Client}/${targetViews[i].map}`.replace(
        "///",
        "/"
      );
    }
    ...
    return _.map(targetViews, (view) => ({
      fullPath: path.join(client.Root, view.name),
      id: `Perforce-${view.name}`,
    }));
  }
  ...
  public async getChangelistForFile(fullPath: string): Promise<string> {
    ...
    const client = this.getCurrentClient();
    const workspace = _.find<IPerforceWorkspace | null>(
      workspaces,
      (ws) => fullPath.includes(ws.Root) && client.Owner === ws.Owner
    );
    if (workspace) {
      return (await this.p4.cmd(`changes -m1 @${workspace.client}`))?.stat?.[0]
        ?.change;
    }
    return null;
  }

  public async isSameRemoteOrigin(
    filePath: string,
    remoteOrigin: string
  ): Promise<boolean> {
    // Removing the "Perforce://" prefix. A remote origin will look like Perforce://MyDepot/MySubDepot
    const remoteOriginWithoutPrefix: string = _.last(
      _.split(remoteOrigin, "://")
    );
...
export const changePerforceManagerSingleton = (
