...
export const logMiddleware: IMiddlewareFunction = async (
  resolve,
  root,
  args,
  context,
...
export const resolveRepoFromId: IMiddlewareFunction = (
  resolve,
  parent,
  args,
  context,
  ...
  const targetRepo = _.find(
    repos,
    (rep) => rep.id.toLowerCase() === repoId.toLocaleLowerCase()
  );
  if (!targetRepo) {
...
export const filterDirTraversal: IMiddlewareFunction = (
  resolve,
  parent,
  ...
  if (isDirTraversal(repo.fullpath, targetPath)) {
    throw new GraphQLError(
      `directory traversal detected. "${targetPath}" does not start with ${repo.fullpath}`
    );
  }
  return resolve(parent, args, context, info);
};
...
export const authenticateController: AuthenticateController = (
  token,
  userId
) => {
  ...
  return async (req, res) => {
    ...
    if (!_.includes(supportedEnvs, env)) {
      res
        .status(400)
        .send(
          `expected env param to be one of [${supportedEnvs}] but got ${
            env || "nothing"
          }`
        );
      return;
    ...
    try {
      ...
      ipcRenderer.send("track", "error-open-chrome", {
...
export const authenticateControllerV2: AuthenticateControllerV2 = (
...
type ConfigureFirstTimeSettings = (
  firstTimeLaunch: boolean,
  serverStartedAt: Date,
  reconfigure: (id: string, site: string) => void
) => RequestHandler;

export const configureFirstTimeSettings: ConfigureFirstTimeSettings = (
  firstTimeLaunch,
  serverStartedAt,
  reconfigure
) => {
  return async (req, res) => {
    ...
    const serverUptimeInMillis =
      new Date().getTime() - serverStartedAt.getTime();
