...
export const resolvers = {
  ...
  Mutation: {
    ...
    getGitRepo: async (
      parent: any,
      args: { sources: [{ repoUrl: string; commit: string }] },
      context: { onAddRepoRequest: onAddRepoRequestHandler }
    ): Promise<OperationStatus> => {
      ...
      if (await isGitFolderBiggerThanMaxSize()) {
        ...
        const tmpDirs = _.filter(subDirs, (dir) =>
          dir.includes(TMP_DIR_PREFIX)
        );
      ...
      const duplicates = _.keys(
        _.pickBy(_.groupBy(args.sources, "repoUrl"), (d) => d.length > 1)
      );
      ...
      const addRepoPromises = _.map(args.sources, async (repo) => {
        ...
        try {
          const cloneDir = await cloneRemoteOriginWithCommit(
            repo.repoUrl,
            repo.commit,
            _.some(duplicates, (d) => d === repo.repoUrl)
          );
          ...
          return {
            ...
            reason: didAddRepo
              ? undefined
              : `Failed to add repo on folder ${cloneDir}`,
          };
        } catch (e) {
          notify(e);
          return {
